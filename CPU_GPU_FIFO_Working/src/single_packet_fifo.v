...
/*
 ...
 // REG INTERFACE WILL BE EXPOSED PORTS
   reg [7:0] pkt_start_addr, pkt_end_addr;
	reg [7:0] start_payload_data;
  assign debug_payload_start = start_payload_data;
  
  wire [31:0] active_offset = (port_master == 2'b10) ? gpu_addr : cpu_addr_in;
  wire [31:0] calculated_addr = active_offset + start_payload_data;
  assign ram_addra = (port_master == 2'b00) ? fifo_addr_in : calculated_addr;
  ...
  assign ram_dina   = port_master == 2'b10 ? {gpu_in_ctrl, gpu_in_data} :port_master == 2'b01 ? {cpu_in_ctrl, cpu_data_in}  : {in_ctrl, in_data};
...
// debug versions of the ram addrb/enb
  assign ram_addrb  = debug_dmem ? debug_dmem_addr : (port_master == 2'b00 ? fifo_addr_in : 8'b0);
  assign ram_enb    = debug_dmem ? 1'b1 : (state == 3'b100 ? 1'b1  : 1'b0); 
