...
//not being used bfloat16add adder(.clk(clk),.rst(rst),
	//		.a(a),
	//		.b(b_neg_or_pos),
	//		.out(add_out));

wire[15:0] mult_out;
//not being used bfloat16mult multunit(.clk(clk),
//	.a(a),.b(b),.out(mult_out));

wire[15:0] tensor_out;
//not being used MAC_unit mac(.clk(clk),.rst(rst),.a(a),.b(b),.c(c),.z(tensor_out));

wire [15:0] relu_out;
//not being used relu_gpu_unit relu_unit(.clk(clk),.rst(rst),.a(a), .z(relu_out));
